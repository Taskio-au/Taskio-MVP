...
**CODE / CLIENT-INITIALIZATION BLOCKER.** App Check is initialized before the Firestore and Storage clients are created, but the first App Check token is not acquired before the first Firestore or Storage operation can run. That is a startup race after production enforcement. Production enforcement must not proceed until a GREEN remediation (**P05G1A**) proves safe first-token readiness, or another SDK-supported equivalent.
...
**Future sequence.** **P05G1A** GREEN first-token remediation, then **P05G1B** GREEN browser/API and proof-surface plan. Only after both pass may RED enforcement be approved. P07’s App Check blocker closes only when P05 production PASS is recorded. P07 itself stays open. Hosting stays maintenance. `pilotSettings` stays absent. Stripe stays disabled. Production stays **FROZEN**.
