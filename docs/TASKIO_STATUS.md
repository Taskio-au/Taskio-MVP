...
- Staging Cloud Run `taskio-api-staging` (closed signup, CORS for staging Hosting + localhost) and staging Hosting SPA are live. Serving API **`taskio-api-staging-54aed8b` 100%**. Hosting **`70429316be0dd106`**. The accepted invite-only landing redesign is on origin/develop (`e8efcbe`, CI `33505435770`) and is **not** on staging Hosting. B4A–G and P02A hosted journeys **PASS**. P04/P05 application code is on origin; staging GA4 and App Check activation **not started**. Production SPA is not restored.
