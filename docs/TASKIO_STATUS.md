...
**Last updated:** 26 September 2026 (P05G3 production App Check token proof PASS; enforcement still OFF)
...
- Production App Check has one registered reCAPTCHA Enterprise provider. **P05G3 PASS** for one production token on `https://taskio.com.au/appcheck-proof/` (Hosting `6e7650bff70d9d48`). The page showed **SIGNED IN**, **Admin YES**, and **App Check READY**, then the operator signed out. Firestore and Storage proofs were **NOT RUN**. Firestore, Storage, and Auth enforcement remain OFF. `/` remains the maintenance page. The proof path stays deployed. This is not a P05 production PASS.
...
| P05 | **STAGING PASS / PRODUCTION PENDING** — **P05G1A–G3 COMPLETE**. **P05G3 PASS** is token-only on Hosting `6e7650bff70d9d48`. Firestore/Storage proofs not run. Enforcement still OFF. `/` remains maintenance. P05G4–P05G6 need separate RED approvals. Not a P05 production PASS | Yes, until production PASS |
