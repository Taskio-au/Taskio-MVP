...
**Last updated:** 25 September 2026 (P05G2 production App Check registration complete; proof artifact local only)
...
- Production App Check now has one registered reCAPTCHA Enterprise provider on the matched existing Web app. Firestore, Storage, and Auth enforcement remain OFF. The temporary `/appcheck-proof/` artifact is prepared locally and not deployed.
...
| P05 | **STAGING PASS / PRODUCTION PENDING** — **P05G1A, P05G1B, and P05G2 COMPLETE**. Production Enterprise provider registered; Firestore/Storage/Auth enforcement still OFF. Narrow `/appcheck-proof/` surface prepared locally, not deployed. P05G3–P05G6 require separate RED approvals. Firestore + Storage only | Yes, until production PASS |
