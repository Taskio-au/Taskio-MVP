...
**Last updated:** 26 September 2026 (P05G3 failed safely and rolled back; P05G3B proof CSP fix local only)
...
- Production App Check has one registered reCAPTCHA Enterprise provider. Firestore, Storage, and Auth enforcement remain OFF. **P05G3 FAILED SAFELY / ROLLED BACK:** the proof page’s CSP blocked `https://apis.google.com/js/api.js` and Firebase Auth returned `auth/internal-error`. Live Hosting is maintenance-only again (`cffca9d87ce03901`). **P05G3B** permits that host in the local proof-page `script-src` only; that fix is not deployed.
...
| P05 | **STAGING PASS / PRODUCTION PENDING** — **P05G1A, P05G1B, and P05G2 COMPLETE**. **P05G3 FAILED SAFELY / ROLLED BACK** (`auth/internal-error`; proof CSP blocked `https://apis.google.com/js/api.js`). **P05G3B** local `script-src` fix is not deployed. Firestore/Storage/Auth enforcement still OFF. Live Hosting is maintenance-only. Not P05G3 PASS. | Yes, until production PASS |
