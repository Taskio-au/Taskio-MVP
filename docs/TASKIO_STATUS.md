...
**Last updated:** 26 September 2026 (P05G4 production Firestore App Check PASS; Storage and Auth still OFF)
...
- Production App Check has one registered reCAPTCHA Enterprise provider. **P05G4 PASS:** Firestore is `ENFORCED`. A valid admin read of missing `adminDailyChecklist/appcheck-proof` returned **MISSING**; missing and invalid App Check each returned **403**. No document was created. Storage proof was **NOT RUN**. Storage and Auth remain `UNENFORCED`. `/` remains the maintenance page. The proof path stays deployed on Hosting `6e7650bff70d9d48`. This is not a P05 production PASS.
...
| P05 | **STAGING PASS / PRODUCTION PENDING** — **P05G1A–G4 COMPLETE**. **P05G4 PASS:** production Firestore App Check `ENFORCED`; valid missing-document read plus missing/invalid App Check **403**. Storage proof not run. Storage and Auth `UNENFORCED`. `/` remains maintenance. P05G5–P05G6 need separate RED approvals. Not a P05 production PASS | Yes, until production PASS |
...
P01 and P02 staging TEST money-path proofs are **COMPLETE**. P03 is **PRODUCTION PASS**; its P07 email blocker is **CLOSED**. P04 and P05 remain **STAGING PASS / PRODUCTION PENDING**. **P05G4 PASS:** production Firestore App Check is `ENFORCED`. Storage proof was not run. Storage and Auth remain `UNENFORCED`. P07 is **OPEN / REMEDIATION IN PROGRESS** (F1–F5B **COMPLETE**; G1–G3 **COMPLETE**; **P07H2 PRODUCTION FIRESTORE + STORAGE RULES PROMOTION COMPLETE**; fail-closed API live on `taskio-api-00008-zir`; staging CSP **ENFORCED**; not PASS). Remaining P07 blockers: production Auth signup path; production App Check Storage; production GA4 / P04 PASS or explicit owner OFF; production Stripe live when serving real money. Firestore App Check is `ENFORCED`. Overall legal pickup remains **P06**. P09 **BLOCKED BY P06**. P08/P10 are not started. READY TO OPEN remains **impossible**.
