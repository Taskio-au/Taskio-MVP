...
**2026-09-24 P03G1B staging transactional-email proof (AMBER `taskio-v2-staging`): PASS.** `verifyTransactionalEmail` deployed alone, `australia-southeast1`, then left **INERT** (`EMAIL_PROOF_ENABLED=false`). Existing staging admin token. Unauthenticated **401**. Gate-off admin **404 proof_disabled**. One HTTP **200** send. Log `transactional_email_proof_sent`. Provider accepted **YES**. Owner mailbox **RECEIVED**. Final admin check **404 proof_disabled**. Customer emails **0**. Application writes **0**. Production **0**. Existing E01 `EMAIL_ENABLED=true` unchanged. **P03G2 ELIGIBLE FOR SEPARATE RED OWNER APPROVAL. NOT APPROVED.** P03 remains **STAGING PASS / PRODUCTION PENDING**. P07 email blocker still waits on production PASS.

**2026-09-24 P03G1A safe email verification function (GREEN local code):** `verifyTransactionalEmail` added. Gate `EMAIL_PROOF_ENABLED` (exact `true` only). Recipient secret name `EMAIL_PROOF_RECIPIENT`. Customer mail remains `EMAIL_ENABLED`. Local email tests **PASS**. Staging proof is the **P03G1B PASS** checkpoint above. **P03G2 ELIGIBLE / NOT APPROVED.**

**2026-09-24 P03G1 production transactional-email plan (GREEN local docs):** Plan in `docs/TRANSACTIONAL_EMAIL.md`. Provider is nodemailer SMTP (Postmark host), not a Postmark HTTP client. E01 is the only staging-proven path. No write-free proof function exists. **P03G2 BLOCKED** until a GREEN verification function (disabled by default, fixed subject/body, not an arbitrary-email admin endpoint) is in the repo and has passed local and staging validation. Customer-facing `EMAIL_ENABLED` stays false. **P03 PRODUCTION EMAIL PLAN: COMPLETE.** Future **P03G2** RED / owner approval required. **NOT APPROVED.** No cloud query, secret read, or email send. P03 remains **STAGING PASS / PRODUCTION PENDING**. P07-07 still consumes that production PASS.
...
| P07 | Production security and configuration | **OPEN / REMEDIATION IN PROGRESS.** **P07F1–F5B COMPLETE.** **P07G1–G3 COMPLETE.** **P07H1–H2 COMPLETE** (`docs/P07_SECURITY_CONFIG_REMEDIATION.md` §47). Expertise fail-open **FIXED + STAGING PROVEN + PRODUCTION LIVE** (`taskio-api-00008-zir` / `57505d0`). Production Firestore + Storage rules **LIVE**. Compatibility **REVIEW COMPLETE**. **Not PASS.** Remaining: Auth signup, App Check, P03 email, P04 GA4 after P06, Stripe live with P10, Hosting with SPA restore. Extra Functions / `nodemailer` / `helloTaskio` / Gmail Editor are **not** automatic P07 PASS blockers. `disabledUserSignup=true` still blocks brand-new Firebase users. See `docs/LAUNCH_READINESS.md`. |
