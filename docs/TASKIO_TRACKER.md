...
**2026-09-25 P05G1A App Check first-token verification (GREEN local docs): RESOLVED BY VERIFICATION.** Firebase JS SDK 12 obtains the App Check token inside Firestore and Storage before protected requests. No startup `getToken()` gate and no source change. The earlier client-initialization race is not a code defect. **SEPARATE PRODUCTION BROWSER/API ARCHITECTURE BLOCKER** remains: the SPA needs `REACT_APP_API_BASE_URL`, the production API is IAM-private, and App Check does not solve that. Next GREEN work is **P05G1B**. RED enforcement is not approved. Production site-key registration stays **MISSING**. GA4 stays off. Auth signup stays disabled. Hosting stays maintenance. P05 remains **STAGING PASS / PRODUCTION PENDING**.
