...
| P07 | Production security and configuration | **OPEN / REMEDIATION IN PROGRESS.** P07A/P07B/P07C complete; P07D1 prepared; P07D2A **proven** (`docs/P07_SECURITY_CONFIG_REMEDIATION.md` §29). f04d has **no active Taskio workflow dependency** and is **READY FOR SEPARATE AMBER REVOCATION**. Historical cloud use **UNKNOWN**. AMBER-D1B not executed. **Not PASS.** Secrets, historical credential confirmation, IAM, production App Check/analytics/email, Firebase/Hosting/API audit, security scans, **and production Auth configuration for the approved homeowner signup path** remain. Current cloud `disabledUserSignup=true` blocks brand-new Firebase users; P07 cannot PASS public OPEN while that remains. Remaining execution is RED. See `docs/LAUNCH_READINESS.md`. |
