...
| P07 | Production security and configuration | **OPEN / REMEDIATION IN PROGRESS.** P07A/P07B/P07C complete; P07D1 prepared; P07D2A proven; P07D2B **AMBER-D1B COMPLETE**; AMBER-E2A+E2B+E2C **COMPLETE** (`docs/P07_SECURITY_CONFIG_REMEDIATION.md` §35): staging rules/API/Hosting current; CSP Report-Only. **Not PASS.** Remaining: CSP enforce decision, production Auth/App Check/email/analytics/Stripe/Storage rules/Hosting, Functions remaining highs, legacy Expert review. Current cloud `disabledUserSignup=true` blocks brand-new Firebase users; P07 cannot PASS public OPEN while that remains. See `docs/LAUNCH_READINESS.md`. |
...
- Live SPA version: `fdc32b272f51d9e0` (P07E2C current HEAD frontend; bundle `main.068025df.js`; CSP Report-Only). Previous proven header-only release `e97a303dcf995e37` / `main.70b28def.js` retained as Hosting rollback target only.
