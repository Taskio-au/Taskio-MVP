...
| P07 | Production security and configuration | **NOT STARTED.** Secrets, historical credential remediation, IAM, production App Check/analytics/email, Firebase/Hosting/API audit, security scans, **and production Auth configuration for the approved homeowner signup path**. Current cloud `disabledUserSignup=true` blocks brand-new Firebase users; P07 cannot PASS public OPEN while that remains. Execution is RED. See `docs/LAUNCH_READINESS.md`. |
