...
**2026-09-25 P05G1B production proof-surface plan (GREEN local docs): COMPLETE.** P05 uses a narrow `/appcheck-proof/` page while `/` stays maintenance. **API BLOCKS P05: NO.** **API BLOCKS FULL SPA RESTORATION: YES.** That blocker moves to **P10** and stays open. Future SPA target is Hosting `/api/**` to a public-invokable `taskio-api` (OPTION B), only after an Authorization-header proof. Hosting rewrite is not an IAM-preserving proxy. Next work is **P05G2** RED, not started. Production site-key registration stays **MISSING**. GA4 stays off. Auth signup stays disabled. Hosting stays maintenance. P05 remains **STAGING PASS / PRODUCTION PENDING**.
