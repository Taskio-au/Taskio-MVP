...
| P05 | App Check | **PRODUCTION PASS.** Application/local/CI and staging proofs pass. Production Firestore + Storage `ENFORCED` and proven; missing/invalid denied; synthetic Storage object deleted; proof surface removed on Hosting `c42a0cac1cc5b789`. Auth Monitoring / `UNENFORCED` — out of approved MVP scope. |
...
| P07 | Production security and configuration | **OPEN / REMEDIATION IN PROGRESS.** **P07F1–F5B COMPLETE.** **P07G1–G3 COMPLETE.** **P07H1–H2 COMPLETE** (`docs/P07_SECURITY_CONFIG_REMEDIATION.md` §47). Expertise fail-open **FIXED + STAGING PROVEN + PRODUCTION LIVE** (`taskio-api-00008-zir` / `57505d0`). Production Firestore + Storage rules **LIVE**. Compatibility **REVIEW COMPLETE**. **P07 production-email and App Check blockers CLOSED** by P03 and P05 production PASS. **Not P07 PASS.** Remaining blockers: production Auth signup path; production GA4 / P04 production PASS or explicit owner OFF decision; production Stripe live configuration when serving real money. Extra Functions / `nodemailer` / `helloTaskio` / Gmail Editor are **not** automatic P07 PASS blockers. `disabledUserSignup=true` still blocks brand-new Firebase users. See `docs/LAUNCH_READINESS.md`. |
...
| P10 | Production acceptance | **NOT STARTED.** End-to-end production money loop, **brand-new homeowner authenticate-and-post proof**, refund, failure paths, integrations, device/browser, accessibility, performance, smoke checklist. Existing invited/synthetic users are not enough for public OPEN. RED. Do not run while only editing the tracker. See `docs/LAUNCH_READINESS.md`. |
...
| P05 | Staging App Check validation + production enforcement decision | **PRODUCTION PASS.** Firestore + Storage enforced/proven in production; Auth Monitoring / `UNENFORCED`; temporary proof surface removed. See `docs/APP_CHECK.md`. |
...
| P07 | Production security and configuration | **OPEN / REMEDIATION IN PROGRESS.** P07A audit complete; P07B local hardening in repo. **Not PASS.** RED to execute remaining production items. See `docs/P07_SECURITY_CONFIG_REMEDIATION.md`. |
