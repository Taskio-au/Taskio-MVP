...
| P07 | Production security and configuration | **OPEN / REMEDIATION IN PROGRESS.** **P07F1–F5B COMPLETE.** **P07G1 REMAINING-BLOCKER RECONCILE COMPLETE.** **P07G2 PRODUCTION API PROMOTION PLAN COMPLETE** (`docs/P07_SECURITY_CONFIG_REMEDIATION.md` §44). Expertise fail-open **FIXED + STAGING PROVEN**. Compatibility **REVIEW COMPLETE**. Production fail-closed API **not deployed**. Future **P07G3** is **RED / OWNER APPROVAL REQUIRED**. **Not PASS.** Canonical remainder: fail-closed API deploy, production rules, Auth signup, App Check, P03 email, P04 GA4 after P06, Stripe live with P10, Hosting with SPA restore. Extra Functions / `nodemailer` / `helloTaskio` / Gmail Editor are **not** automatic P07 PASS blockers. `disabledUserSignup=true` still blocks brand-new Firebase users. See `docs/LAUNCH_READINESS.md`. |
