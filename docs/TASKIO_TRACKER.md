...
| P07 | Production security and configuration | **OPEN / REMEDIATION IN PROGRESS.** **P07F1–F5B COMPLETE.** **P07G1–G3 COMPLETE** (`docs/P07_SECURITY_CONFIG_REMEDIATION.md` §45). Expertise fail-open **FIXED + STAGING PROVEN + PRODUCTION LIVE** (`taskio-api-00008-zir` / `57505d0`). Compatibility **REVIEW COMPLETE**. **Not PASS.** Remaining: production Firestore+Storage rules, Auth signup, App Check, P03 email, P04 GA4 after P06, Stripe live with P10, Hosting with SPA restore. Extra Functions / `nodemailer` / `helloTaskio` / Gmail Editor are **not** automatic P07 PASS blockers. `disabledUserSignup=true` still blocks brand-new Firebase users. See `docs/LAUNCH_READINESS.md`. |
