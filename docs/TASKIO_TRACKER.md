...
**2026-09-25 P05G1 production App Check and SPA readiness plan (GREEN local docs): COMPLETE.** Provider is reCAPTCHA Enterprise via Firebase `initializeAppCheck`, proven on staging for Firestore and Storage. Auth App Check stays out of MVP scope. Production site key is not provisioned. Production Hosting CSP is not the staging SPA policy. Future order: App-Check-capable SPA while enforcement is off, then Firestore and Storage enforcement. Auth signup stays disabled. GA4 stays off. Execution is **RED / OWNER APPROVAL REQUIRED**. Not started. P05 remains **STAGING PASS / PRODUCTION PENDING**.
