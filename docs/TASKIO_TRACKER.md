...
**2026-09-26 P07 owner decision + canonical reconciliation (GREEN local): P07 PASS / CLOSED.** Owner decisions: production GA4 remains **OFF through P06/P09** and the explicit production-OFF exception satisfies the P04/P07 analytics condition; Firebase Auth signup remains **DISABLED** while frozen, with controlled enable/new-user/rollback proof moved to **P10** and sustained availability to **P11**; Stripe LIVE remains **DISABLED**, with configuration and live money acceptance moved to **P10 after P06/P09** and sustained operation to **P11**. P03 is **PRODUCTION PASS / COMPLETE / CLOSED**. P05 is **PRODUCTION PASS / COMPLETE / CLOSED**: Firestore + Storage `ENFORCED`, Auth Monitoring / `UNENFORCED`, proof surface removed. P07 evidence is complete for secrets/runtime isolation, fail-closed API, current Firestore/Storage rules, signup-path readiness, email, App Check, and legacy Expert fail-closed compatibility. Final P07 closure required **no RED production mutation**. P06 remains OPEN; P09 remains blocked by P06; P10 is NOT STARTED; P11 is BLOCKED. Read-only Auth inspection exposed sensitive configuration material in a private local tool transcript; no value was copied into repository evidence. Containment/rotation assessment remains an operator security follow-up. No cloud query was rerun. No deploy, Auth, IAM, App Check, Stripe, GA4, email, `pilotSettings`, data, commit, or push mutation.

**2026-09-26 P05G6 production proof-surface removal (RED `taskio-v2`): PASS. P05 PRODUCTION PASS.** Removed `maintenance/appcheck-proof/` and its path-specific CSP; root maintenance files stayed byte-identical. Focused removal tests **4/4 PASS**; launch-status tests **12/12 PASS**; deploy artifact scan found no proof/Firebase runtime. One Hosting-only deploy released version **`c42a0cac1cc5b789`**, release **`1790412206562000`**, at `2026-09-26T08:43:26.562Z`; exactly five maintenance files. Live `/`, `/appcheck-proof/`, and `/appcheck-proof/app.js` serve identical maintenance HTML on `taskio.com.au` and both Firebase default hosts, with no proof content or proof CSP. Final App Check Console: Firestore **Enforced**, Storage **Enforced**, Authentication **Monitoring**. The proof object remains absent. P07 App Check blocker **CLOSED**; P07 remains open for other blockers. No rules, Auth, IAM, API, Functions, Stripe, GA4, customer/business-data, commit, or push change. Production remains **FROZEN / NOT OPEN**.

**2026-09-26 P05G5 production Storage App Check (RED `taskio-v2`): PASS.** Storage was `ENFORCED` at `2026-09-26T08:13:51.422063Z`; Firestore stayed `ENFORCED`; Auth stayed Monitoring / `UNENFORCED`. Existing Google admin verified `admin=true`; App Check **READY**. Exactly one 68-byte synthetic PNG upload to `profilePhotos/4wX3ROz5O4RVtAK59puGGxrqS813/appcheck-proof.png` succeeded; valid metadata read succeeded; missing and invalid App Check each returned **401**. The authorised operator CLI deleted exactly that object; exact-path verification returned **404**. Operator signed out. No rollback or unrelated production mutation. At this checkpoint P05 still awaited G6 proof-surface removal.

**2026-09-26 P05G4 production Firestore App Check (RED `taskio-v2`): PASS.** Firestore enforcement was set to `ENFORCED` at `2026-09-26T07:34:22.237215Z`. Storage and Auth stayed `UNENFORCED`. Hosting stayed `6e7650bff70d9d48`. The existing admin signed in on `https://taskio.com.au/appcheck-proof/`. The valid read of absent `adminDailyChecklist/appcheck-proof` returned **MISSING**. The same read without an App Check header returned **403**, and with an invalid App Check header returned **403**. No document was created. No token value was recorded. Storage proof **NOT RUN**. The operator signed out. No Hosting deploy, rules change, Auth change, IAM change, or API change in this proof. **Not a P05 production PASS. P05 remains STAGING PASS / PRODUCTION PENDING. Firestore stays ENFORCED. P07 App Check blocker OPEN until Storage enforcement. Next: P05G5 under separate RED approval. Production remains FROZEN / NOT OPEN.**

**2026-09-26 P05G3 production token proof (RED `taskio-v2`): PASS, token only.** After the CSP rollback recorded below, commit `048ef1a` added `https://apis.google.com` to the proof-page `script-src` only. CI [36225482223](https://github.com/Taskio-au/Taskio-MVP/actions/runs/36225482223) **SUCCESS**. One Hosting deploy of `firebase.maintenance.json` released version `6e7650bff70d9d48` (`1790406359037000`, `2026-09-26T07:05:59.037Z`). `/` stayed “Taskio is almost ready”. The existing admin signed in with Google on `https://taskio.com.au/appcheck-proof/`. The page showed **Admin YES** and **App Check READY**, then the operator signed out. No token value was displayed or stored. Firestore proof **NOT RUN**. Storage proof **NOT RUN**. Firestore, Storage, and Auth remained `UNENFORCED` after sign-out. `/appcheck-proof/` remains deployed. No Auth, IAM, API, rules, Stripe, GA4, or `pilotSettings` change. **Not a P05 production PASS. P05 remains STAGING PASS / PRODUCTION PENDING. P07 App Check blocker OPEN. Next: P05G4 under separate RED approval. Production remains FROZEN / NOT OPEN.**

**2026-09-26 P05G3 production proof Hosting (RED `taskio-v2`): FAILED SAFELY / ROLLED BACK. P05G3B local CSP fix prepared, not deployed.** The deployed proof page called `signInWithPopup` and Firebase Auth returned `auth/internal-error` because the path CSP blocked `https://apis.google.com/js/api.js` (`script-src-elem`). No Google popup, no account chooser, no Firestore proof, and no Storage proof. Enforcement stayed `UNENFORCED`. Hosting was restored to maintenance-only version `cffca9d87ce03901` (release `1790403428645000`). Local `firebase.maintenance.json` now permits `https://apis.google.com` on the proof-page `script-src` only. `https://accounts.google.com` was not added. Root headers, Auth code, and App Check code are unchanged. Not committed. Not pushed. **P05G3 is not PASS. P05 remains STAGING PASS / PRODUCTION PENDING. Production Hosting is maintenance-only. Production remains FROZEN / NOT OPEN.**

**2026-09-25 P05G2 production Enterprise registration + local proof artifact (RED `taskio-v2`): COMPLETE.** Existing active Web app **Taskio Web** matched the repo app ID. Pre-state: no Enterprise site key, Enterprise-key inventory empty, reCAPTCHA Enterprise API already enabled, Firestore/Storage/Auth `UNENFORCED`. Created exactly one WEB/SCORE key named **Taskio Production App Check**, domain verification enabled and allow-all-domains false, restricted only to `taskio.com.au`, `www.taskio.com.au`, `taskio-v2.web.app`, and `taskio-v2.firebaseapp.com`. Registered it only to the matched production Web app; read-back TTL `3600s`, score threshold `0.5`; all enforcement remains `UNENFORCED`. Prepared `maintenance/appcheck-proof/` locally with admin-gated manual token acquisition and hard-disabled exact-path Firestore/Storage controls. Path-specific CSP prepared. Root maintenance files unchanged. No Hosting deploy, token/browser proof, data request/write, enforcement, Auth/IAM/API/Functions/email/Stripe/GA4/`pilotSettings` change, commit, or push. **P05 remains STAGING PASS / PRODUCTION PENDING. P07 App Check blocker OPEN. P10 browser/API blocker OPEN. Next: P05G3 under separate RED approval.**

**2026-09-25 P05G1B production proof-surface plan (GREEN local docs; historical checkpoint superseded by P05G2 above): COMPLETE.** P05 uses a narrow `/appcheck-proof/` page while `/` stays maintenance. **API BLOCKS P05: NO.** **API BLOCKS FULL SPA RESTORATION: YES.** That blocker moves to **P10** and stays open. Future SPA target is Hosting `/api/**` to a public-invokable `taskio-api` (OPTION B), only after an Authorization-header proof. Hosting rewrite is not an IAM-preserving proxy. At this checkpoint P05G2 was not started and production registration was missing; P05G2 subsequently completed as recorded above. GA4 stays off. Auth signup stays disabled. Hosting stays maintenance. P05 remains **STAGING PASS / PRODUCTION PENDING**.

**2026-09-25 P05G1A App Check first-token verification (GREEN local docs): COMPLETE.** Firebase JS SDK 12 obtains the App Check token inside Firestore and Storage before protected requests. No startup `getToken()` gate and no source change. The first-token blocker is **CLOSED**.

**2026-09-25 P03G2 production transactional-email proof (RED `taskio-v2`): PRODUCTION PASS.** Existing owner-controlled Google Firebase admin; fresh token proved `aud=taskio-v2`, provider `google.com`, `admin=true`. Production SMTP configured and sender/domain authorised. Both E01 Functions remained active with `EMAIL_ENABLED=false`. `verifyTransactionalEmail` was active; its proof gate was enabled only for exactly one authenticated send. HTTP **200**; `transactional_email_proof_sent` logged exactly once; provider accepted; owner mailbox receipt confirmed. Gate restored to `EMAIL_PROOF_ENABLED=false`; final admin POST **404 `proof_disabled`**; additional sends **0**. Customer emails **0**. Business-data writes **0**. No high-risk privacy/security issue. Proof Function remains **ACTIVE + INERT**. Production remains **FROZEN / NOT OPEN**. **P07 production-email blocker CLOSED; P07 remains OPEN.**

**2026-09-24 P03G1B staging transactional-email proof (AMBER `taskio-v2-staging`): PASS; historical checkpoint superseded by P03G2 above.** `verifyTransactionalEmail` deployed alone, `australia-southeast1`, then left **INERT** (`EMAIL_PROOF_ENABLED=false`). Existing staging admin token. Unauthenticated **401**. Gate-off admin **404 proof_disabled**. One HTTP **200** send. Log `transactional_email_proof_sent`. Provider accepted **YES**. Owner mailbox **RECEIVED**. Final admin check **404 proof_disabled**. Customer emails **0**. Application writes **0**. Production **0**. Existing E01 `EMAIL_ENABLED=true` unchanged. At this checkpoint P03G2 still required separate RED owner approval.

**2026-09-24 P03G1A safe email verification function (GREEN local code; historical checkpoint):** `verifyTransactionalEmail` added. Gate `EMAIL_PROOF_ENABLED` (exact `true` only). Recipient secret name `EMAIL_PROOF_RECIPIENT`. Customer mail remains `EMAIL_ENABLED`. Local email tests **PASS**. Staging proof is the **P03G1B PASS** checkpoint above. P03G2 had not yet been approved at this checkpoint.

**2026-09-24 P03G1 production transactional-email plan (GREEN local docs; historical checkpoint):** Plan in `docs/TRANSACTIONAL_EMAIL.md`. Provider is nodemailer SMTP (Postmark host), not a Postmark HTTP client. At this point E01 was the only staging-proven path and the write-free proof function was still a prerequisite. Customer-facing `EMAIL_ENABLED` stayed false. **P03 PRODUCTION EMAIL PLAN: COMPLETE.** No cloud query, secret read, email send, or deploy occurred in P03G1. P03G2 later completed under the separate approval recorded above.

**2026-09-24 P07H2 production Firestore + Storage rules promotion (RED `taskio-v2`):** Firestore Release `projects/taskio-v2/releases/cloud.firestore` now `rulesets/27e9e017-3172-4e68-a69d-f515755aeef5` (was `7d46b484-b848-4d31-8184-18805641d86b`). Storage bucket `taskio-v2.firebasestorage.app`; Release `…/firebase.storage/taskio-v2.firebasestorage.app` now `rulesets/6e5b1d16-55dc-482b-8313-a420604165d9` (was `932c4f1b-cb8d-46b3-bf14-b53a971bb4f4`). Source SHA-256 matches HEAD. Emulator **26/26 PASS**. Propagation rechecked at 60s. Rollback **not used**. No application writes. App Check / Auth / Hosting unchanged. See `docs/P07_SECURITY_CONFIG_REMEDIATION.md` §47. P07 **OPEN** — **not PASS**.

**2026-09-24 P07H1 production Firestore + Storage rules promotion plan (GREEN local docs):** Plan in `docs/P07_SECURITY_CONFIG_REMEDIATION.md` §46. HEAD `firestore.rules` / `storage.rules` byte-match E2A. Emulator **26/26 PASS**. **RULES FIRST**; Firestore then Storage in the **same RED window**, each validated before the next. Rollback re-points the captured Release to the captured pre-H2 Ruleset (not Git). Storage browser write **not** required for P07-16 (belongs to P10 / P05). **FIRESTORE RULES CODE-READY: YES.** **STORAGE RULES CODE-READY: YES.** Future **P07H2** RED / owner approval required. **NOT APPROVED.** No cloud query/mutation. P07 **OPEN** — **not PASS**.

**2026-09-21 P07G3 production fail-closed API promotion (RED `taskio-v2`):** Source `57505d0`. Cloud Build `6e5f5f31-66c7-4f58-9c0b-cafd9420e7c4`. Image `sha256:712814b084d809370b5e0632fc2356bed4d0e16179bfeba1991894f21a771014`. New revision `taskio-api-00008-zir` tag `g3-57505d0` attested at 0% then **100%**. `taskio-api-00006-puf` retained 0% (`abn-hardened`). `--no-allow-unauthenticated` rewrote IAM policy etag; **effective access unchanged** (still private, no `allUsers`/public invoker). Stripe disabled. AI fallback. Register `signup_disabled`. `pilotSettings` absent. Migration **not** executed. Expert mutation **NONE**. Rollback **not used**. See `docs/P07_SECURITY_CONFIG_REMEDIATION.md` §45. P07 **OPEN** — **not PASS**.

**2026-09-21 P07G2 production fail-closed API promotion plan (GREEN local docs):** Plan in `docs/P07_SECURITY_CONFIG_REMEDIATION.md` §44. Source SHA `57505d0`. Image strategy: fresh production Artifact Registry build (do not copy staging digest). Preserve `taskio-api-00006-puf`. API FIRST before rules. G3 smoke distinguishes Cloud Run IAM (Layer 1 / `X-Serverless-Authorization`) from Taskio application auth (Layer 2 / Firebase `Authorization`). **API PROMOTION CODE-READY: YES.** Future **P07G3** RED / owner approval required. No cloud query/mutation. P07 **OPEN** — **not PASS**.

**2026-09-20 P07G1 remaining P07 blocker reconcile (GREEN local docs):** Canonical remainder in `docs/P07_SECURITY_CONFIG_REMEDIATION.md` §43. Fail-open **FIXED + STAGING PROVEN**. Compatibility **REVIEW COMPLETE**. Production fail-closed API **not deployed**. Next P07 task **P07G2** GREEN plan (not a deploy). P06 remains the legal pickup. P07 **OPEN** — **not PASS**. No cloud query/mutation.

**2026-09-20 P07F5B owner decision OPTION B (GREEN local docs):** `Prod-Expert-08` treated as genuine legacy Expert with unknown requested expertise. Leave fail-closed. No copy approved→requested. No auto-approve/backfill. Migration **not** executed. Owner accepts known technical-eligibility loss until future Expert reconfirmation. Legacy Expert compatibility blocker **RESOLVED**. **FIXED PRODUCTION API: COMPATIBILITY REVIEW COMPLETE** — **not** a deploy approval. Data mutation **NONE**. P07F5A **OWNER REVIEW DOSSIER COMPLETE**. P07F5B **OWNER DECISION COMPLETE — OPTION B**. P07 **OPEN** — **not PASS**. No production mutation.

**2026-09-20 P07F5A Prod-Expert-08 owner-review dossier (GREEN local docs, read-only `taskio-v2`):** Eligibility-change Expert bound by P07F4 operational fingerprint (invites 13 / accepted 10 / quotes 11). Requested missing; 5 canonical approved keys listed without identity. Admin Verify event **2026-04**; five-key approval provenance **unknown**. No explicit synthetic marker. Launch-ready **NO**. Options A–D prepared. Later decided in P07F5B. 6 other owner-review records remain already ineligible under both logics. P07F5A **OWNER REVIEW DOSSIER COMPLETE**. P07 **OPEN** — **not PASS**. No production mutation.

**2026-09-20 P07F4 production Expert compatibility (GREEN local docs, read-only `taskio-v2`):** 38 users / **18** tradies. Fixed effective expertise **0**; launch-ready **0**. Pre-fix technically eligible **1** → fixed **0** (`Prod-Expert-08`, approved-only). Jobs/quotes aggregates for that Expert: invites 13 / accepted 10 / quotes 11. Migration candidates **0**; migration **not** executed. Data remediation: **OWNER REVIEW REQUIRED**. Deploy class **B**. Production writes **0**. P07F4 **PRODUCTION READ-ONLY AUDIT COMPLETE**. P07 **OPEN** — **not PASS**.

**2026-09-20 P07F3 AMBER-P07F3 (STAGING API):** Cloud Build `24c0d8fd-37b4-4766-ac87-9dc3c33b7623` image `taskio-api@sha256:ebfecf48…` from `57505d0`. New revision `taskio-api-staging-00072-vur` tag `f3-57505d0` attested at 0% then **100%**. Expert-01 compatible; GET `/api/me` and `/api/tradie/profile` did not change Firestore `updateTime` or approval fields. Migration **not** executed. Hosting still `b963ae61de25da7e` / `main.068025df.js`; CSP **ENFORCED**. `00070-dur` and `54aed8b` retained at 0%. Rollback not used. P07F3 **STAGING DEPLOYMENT COMPLETE**. P07 **OPEN** — **not PASS**. No production mutation.

**2026-09-20 P07F2 Expert expertise fail-closed (GREEN local code+tests):** Fail-closed `hasApprovedMarketplaceExpertise` / effective = approved ∩ requested when both are arrays. Removed GET `/api/me` and `/api/tradie/profile` persist-on-read approval. Migration requires `verified===true` and a canonical requested array; not run. Backend Jest **1058/1058**. Staging data still **not** remediating. Later deployed in P07F3. Production Expert audit **PENDING**. P07F2 **REMEDIATION COMPLETE**. P07 **OPEN** — **not PASS**.

**2026-09-20 P07F1 staging legacy Expert eligibility audit (GREEN local docs):** Read-only `taskio-v2-staging`. 4 users / **1** Expert (`tradie`). Class **C**: technically eligible, **not** launch-ready (`NOT_ACCEPTING_JOBS`, `NO_SERVICE_AREA`). Launch-ready **0**. No staging data remediation. Code hole `hasApprovedMarketplaceExpertise` (both fields missing ⇒ true) documented **HIGH**, **not patched**. Live Admin supply API **NOT EXECUTED**. Production Expert audit **PENDING**. Hosting/API/rules/CSP unchanged. P07F1 **AUDIT COMPLETE**. P07 **OPEN** — **not PASS**. No cloud mutation.

**2026-09-20 P07D3 AMBER-D3 (STAGING CSP ENFORCED):** Header-only Hosting deploy of identical E2C SPA (`main.068025df.js` SHA match). Converted proven Report-Only policy to `Content-Security-Policy`. New version **`b963ae61de25da7e`**. Report-Only header removed. Public desktop/mobile + existing synthetic homeowner/Expert/admin logins executed under enforcement; **0** CSP blocked resources. Rollback **not** used; keep `fdc32b272f51d9e0`. API `00070-dur` 100%; `54aed8b` 0%. AMBER-D3 **COMPLETE**. P07 **OPEN** — **not PASS**. No production mutation.

**2026-09-20 P07E2C AMBER-E2C (STAGING Hosting):** Clean worktree HEAD `5cc5cac` build `main.068025df.js` / `main.5e46c8ad.css` (same as P07E1; docs-only since `04951a4`). Deployed Hosting-only to `taskio-v2-staging` via wrapper. New version **`fdc32b272f51d9e0`**. Live index current bundle. P07B headers + CSP Report-Only retained; no enforced CSP; **0** corrective redeploys. Public desktop/mobile CLOSED/WAITLIST; `/tradie/signup` → `/expert-waitlist`; invalid waitlists did not write. Existing synthetic homeowner/Expert/admin read-only logins **executed**. API `00070-dur` 100%; `54aed8b` kept 0%. Rules/App Check/Auth/`pilotSettings` unchanged. Rollback **not** used. AMBER-E2C **COMPLETE**. P07 **OPEN** — **not PASS**. No production mutation.

**2026-09-20 P07E2B AMBER-E2B (STAGING API):** Cloud Build `617d9d4c-0ecc-4ed3-93f9-604d18bf10c0` image `taskio-api@sha256:6c29aea9…` from `04951a4`. New revision `taskio-api-staging-00070-dur` tag `e2b-04951a4` attested at 0% (health/pilot-status CLOSED+WAITLIST, auth/admin/AI/CORS). Then **100%** traffic. Previous `54aed8b` retained at 0% (`checkout-url`). Hosting still `e97a303dcf995e37` / `main.70b28def.js`. Rules unchanged E2A. Auth signup disabled. `pilotSettings` absent. No business writes. Rollback not used. AMBER-E2B **COMPLETE**. E2C Hosting pending. P07 **OPEN** — **not PASS**. No production mutation.

**2026-09-20 P07E2A AMBER-E2A (STAGING rules only):** Deployed HEAD `firestore.rules` + `storage.rules` to `taskio-v2-staging` only. Firestore `b8b8e7f2…` → `28c69372-2f82-4171-a7ad-379b0335b5a5`. Storage `64c5b44b…` → `a0736ecb-e3bb-4573-8608-c1bced82fab8`. Sources match HEAD. Rules tests 26/26. Public `/` `/login` `/post-job` still old invite-only SPA; CSP Report-Only; no App Check console errors. Hosting `e97a303dcf995e37`, API `54aed8b`, webhook `00007-8tx`, Functions quote-email pair unchanged. Auth signup still disabled. `pilotSettings` absent. Rollback not used. AMBER-E2A **COMPLETE**. Remaining E2B API then E2C Hosting. P07 **OPEN** — **not PASS**. No production mutation.
...
**2026-09-19 P07B local security hardening (GREEN local):** Fail-closed `AI_DESCRIPTION_ENABLED` (guest `/api/generate-description` retained); Storage create-only posting photos + 2MB profile bound; conservative Hosting headers (CSP deferred); Functions safe overrides (nodemailer major remaining). P07 **OPEN / REMEDIATION IN PROGRESS** — **not PASS**. P03/P04/P05/P06/P09 unchanged. Pushed with P07B finalizer; CI `35425409205` SUCCESS. Production frozen.

**2026-09-14 P07A production security/config audit (GREEN local docs):** Read-only audit of secrets hygiene, Auth/Expert gates, Admin/public APIs, CORS/limits/headers, rules, App Check, Stripe/email/GA4/AI, logging, CI, kill switches. Matrix `docs/P07_SECURITY_CONFIG_REMEDIATION.md`. Manifest P07 = **OPEN** (audit complete / remediation pending) — **not PASS**. P03/P04/P05 production pending unchanged. P06 **OPEN**. P09 blocked. No cloud mutation. No push/deploy. Production frozen.

**2026-09-14 P06A legal/privacy/trust reconciliation (GREEN local docs):** Current-state inventory after homeowner CLOSED/OPEN/PAUSED and independent Expert OPEN/WAITLIST. Public-claim, payment/escrow, verification, waitlist, processor, refund, and review facts recorded. Solicitor register SR01–SR24 and remediation matrix prepared. Terms/Privacy **not** rewritten. P06 remains **OPEN**. P09 remains **blocked**. Pack: `docs/P06_OWNER_DECISIONS.md`, `docs/P06_SOLICITOR_BRIEF.md`, `docs/P06_REMEDIATION_MATRIX.md`. No product code. No push/deploy. Production untouched.

**2026-09-14 Expert requested vs approved categories + settings field preservation (GREEN local):** Homeowner state and Expert onboarding mode merge independently on `system/pilotSettings`. New Expert signup stores self-selected Phase 1 keys on `expertise` and leaves `expertiseApproved=[]`. Admin Verify explicitly approves currently requested categories; later self-adds stay unapproved until `PUT /api/admin/users/:uid/expertise/approve`. Supply, invite, and quote matching use approved/effective categories. Legacy Experts with existing `expertiseApproved` keep eligibility; no cloud migration. P06 **OPEN**. P09 blocked. Nothing pushed or deployed.
...
> **2026-09-12 checkpoint.** P01 **PASS / COMPLETE**. P02 **COMPLETE**. P03/P04/P05 **STAGING PASS / PRODUCTION PENDING**. P06 **OPEN** (inventory **COMPLETE**; owner facts **COMPLETE**; lean sole-trader controlled pilot as owner working plan, subject to AU solicitor confirmation; Pty Ltd **not** an automatic launch blocker; company conversion deferred unless advised before pilot; insurance = focused minimum-pilot broker/solicitor review, not broad cover as blocker; accounting marketplace/GST **PENDING**; remediation **NOT STARTED**; not PASS). P07–P10 **NOT STARTED**. P09 blocked on P06. P11 **BLOCKED**. Production PRE-LAUNCH FREEZE unchanged.
...
| P03 | Transactional email | **PRODUCTION PASS / COMPLETE / CLOSED.** Staging E01 and guarded proof **PASS**. Production SMTP configured; sender/domain authorised; required E01 runtime deployed with both customer gates `EMAIL_ENABLED=false`. P03G2 used the guarded admin proof Function for exactly one HTTP **200** send; provider accepted and owner receipt was confirmed. Gate restored off; final **404 `proof_disabled`**. Customer emails **0**; business-data writes **0**. `verifyTransactionalEmail` remains **ACTIVE + INERT**. |
| P04 | Privacy-safe analytics | **PASS — OWNER-APPROVED PRODUCTION OFF.** Application/local/CI and staging hosted/Realtime proofs pass. Production GA4 remains **OFF through P06/P09**; no production analytics proof is claimed. If later selected, production enablement/proof belongs to P10 after approved privacy wording. |
| P05 | App Check | **PRODUCTION PASS / COMPLETE / CLOSED.** Application/local/CI and staging proofs pass. Production Firestore + Storage `ENFORCED` and proven; missing/invalid denied; synthetic Storage object deleted; proof surface removed on Hosting `c42a0cac1cc5b789`. Auth Monitoring / `UNENFORCED` — out of approved MVP scope. |
| P06 | Legal review | **OPEN.** Inventory **COMPLETE**. Owner facts **COMPLETE**. **P06A reconciliation PREPARED** (`docs/P06_REMEDIATION_MATRIX.md`). **LEGAL STRUCTURE REVIEW:** required before controlled real-user pilot. Current sole-trader pilot: permitted by owner as working plan, **subject to AU solicitor confirmation**. **COMPANY CONVERSION:** deferred unless required/advised before pilot; must be reconsidered before broader scaling. **INSURANCE:** minimum pilot position must be reviewed with broker/solicitor before real users; broad insurance expansion can scale with traction/risk. Accounting marketplace/GST **PENDING**. Remediation **matrix prepared / implementation not started**. **Not PASS.** Pty Ltd is **not** a tracker launch blocker unless professionally advised. See `docs/P06_OWNER_DECISIONS.md`. Do not start P09 UI until P06 PASS. |
| P07 | Production security and configuration | **PASS / CLOSED.** P07F1–F5B, G1–G3 and H1–H2 are complete. The fail-closed production API is live on `taskio-api-00008-zir`; production Firestore/Storage rules match HEAD; P03 email and P05 App Check are closed; legacy Expert compatibility remains fail-closed. Signup-path security/readiness is complete while cloud signup intentionally stays disabled. Owner GA4-OFF and Stripe-deferred decisions are recorded. Final closure required **no RED production mutation**. |
...
| P10 | Production acceptance | **NOT STARTED.** Open: real SPA restoration; production browser/API architecture; Authorization-header forwarding proof; controlled Auth enable/new-user/rollback proof; LIVE Stripe config and money loop after P06/P09; customer-email journey if required; optional GA4 proof only if later approved; refund/failure/device/accessibility/performance checks. RED. |
| P11 | Controlled launch | **BLOCKED** until **TASKIO FULL LAUNCH READY**. Inner Melbourne as **one zone**. Homeowner **posting stays closed** until the supply gate (~15 launch-ready Experts = technical eligibility + `acceptingJobs` + `serviceAreas[]` + category/geo coverage + other real-user gates + explicit owner activation). Hard 4–5 is per **enabled category**, not per suburb. After-activation floor ~12 → WATCH/PAUSE. See `docs/LAUNCH_READINESS.md` and `docs/P06_OWNER_DECISIONS.md` §5A–§5B. |

**TASKIO FULL LAUNCH READY** = P01 PASS + P02 PASS + P03 production PASS + P04 gate PASS (production proof or explicit owner-OFF exception) + P05 production PASS + P06 PASS + P07 PASS + P08 PASS + P09 PASS + P10 PASS. **P11** is then the controlled-launch execution gate. It cannot start before that definition is true.

**Mandatory before controlled launch:** P01/P02 complete; P03/P05 production PASS; P04 gate PASS; P06; P07; P08; P09; P10. Current remaining gates are P06, P08, P09 and P10.
...
2. Legal/privacy closeout: focused AU solicitor (Stage 1 pilot + Stage 2 scale) + focused insurance broker (minimum sensible pilot cover) + accountant marketplace/GST; then P06 APPROVED; P09 implements only after P06 PASS. Pty Ltd / broad insurance are not automatic blockers unless professionally advised.
3. Production preflight (P03 **PASS**; P04/P05 production PASS plus P07/P08), including separate RED production-analytics and production-App-Check approvals
...
**Exact next pickup:** **P06** focused AU solicitor review (Stage 1 controlled pilot + Stage 2 scale) of `docs/P06_OWNER_DECISIONS.md`, `docs/P06_SOLICITOR_BRIEF.md`, and `docs/P06_REMEDIATION_MATRIX.md`, plus focused insurance-broker discussion (minimum sensible pilot cover) and accountant marketplace/GST confirmation. Owner facts **COMPLETE**. Reconciliation **PREPARED**. Do **not** treat Pty Ltd or broad insurance as automatic blockers. Do **not** mark P06 PASS, start P09 UI/copy, enable Auth App Check, roll back Hosting while Firestore or Storage is ENFORCED, start P02B, start P11, change DNS, enable customer email, or configure production analytics tonight. Production remains frozen.

**Staging Cloud Run (authoritative B4 / current serving):**

- URL: `https://taskio-api-staging-d6mdcsrwea-ts.a.run.app`
- Serving 100%: `taskio-api-staging-54aed8b` (tag `checkout-url`; Checkout Session URL payload)
- Immediate previous known-good: `taskio-api-staging-00063-qak` (tag `stripe-key-rotate`, 0% serving)
- Image: `australia-southeast1-docker.pkg.dev/taskio-v2-staging/taskio-staging/taskio-api@sha256:3d207e01da5209f8a01903b006ed05b9c6d1a5c290e96f065e1dc0f77b42396d` from CI-green `54aed8b`
- Env/config inherited from `00063-qak`: closed signup, TEST Stripe secret `:3`, exact CORS, `FRONTEND_URL`, runtime SA, `invoker-iam-disabled=true`
...
- Open-signup revision `d3o039629` remains 0%
...
- Live SPA version: `b963ae61de25da7e` (P07D3 enforced CSP; same `main.068025df.js` as E2C). Previous Report-Only release `fdc32b272f51d9e0` retained as Hosting rollback. Earlier header-only `e97a303dcf995e37` / `main.70b28def.js` is obsolete for current-SPA rollback.
...
**B4F evidence (2026-08-30) — PASS:**

- Remediation source: `54aed8bee5baf2b088e65c92aa5be54027b3adce` (`fix(checkout): navigate to Stripe Session URL instead of Stripe.js`) **pushed**. CI [`33297343725`](https://github.com/Taskio-au/Taskio-MVP/actions/runs/33297343725) **success** (security-rules, frontend, backend, functions, api-image, webhook-image, browser-smoke).
- Tracker correction `c0a2c04` (`docs: record B4F payment/webhook pass and hosted Checkout handoff gap`) was pushed with that pair; this row supersedes its PARTIAL classification.
- Staging API: Cloud Build `801126b4-83c1-4937-aca2-565872d12852` **SUCCESS** (API-only; builder SA). New revision `taskio-api-staging-54aed8b` attested at 0% (`/health/live` and `/health/ready` 200; env/secrets identical to `00063-qak` including `TASKIO_PUBLIC_SIGNUP_ENABLED=false`, `STRIPE_ENABLED=true`, `STRIPE_EXPECTED_LIVEMODE=false`, `STRIPE_SECRET_KEY` `:3`, exact CORS, `FRONTEND_URL`, runtime SA). Then **100%** traffic to `54aed8b`. `00063-qak` retained tagged at 0%. `d3o039629` remains 0%.
- Staging Hosting: hardened wrapper `--project taskio-v2-staging --config firebase.staging.hosting.json --execute`. New version **`70429316be0dd106`**. Previous **`66888d3b5a527558`**. Bundle `main.535a3e06.js`: no `@stripe/stripe-js`, no `loadStripe`, no `redirectToCheckout`, no `pk_live_`, no `pk_test_`, has `checkoutUrl`, staging API host, noindex/no-store. Landing/login/dashboard did **not** request `js.stripe.com`.
- **B4F payment / webhook core: PASS** (original TSK-5507 AUD 120 on `00063-qak` remains valid payment-integrity evidence; preserved).
- **B4F normal hosted browser handoff: PASS** on **new** job `fu8pWO3D4BY4DCo1s36u` / **TSK-3881** (Hanging / picture frames, Richmond). Admin invited existing synthetic expert. Quote `CqywhWbZZWARfEDBoW6D` AUD **90.00** submitted then **accepted**. Homeowner **Accept & fund** caused API `checkoutUrl` (`https` `checkout.stripe.com` `/c/pay/cs_test_…`) and **automatic** browser navigation to that URL. No publishable-key / Stripe.js Session retrieve. Operator did not copy/paste a Session URL.
- Stripe TEST objects (no secrets): Session `cs_test_a1Z1lRIPA155ZS1XiIhT5W4Q2xxLPIZq8Dk6vQgPoUNkTfxQBg7Q1ZD3xC` complete/paid `livemode=false` AUD 90.00; PI `pi_3UA2jUGdq6QKDpuT1mXg2gyb` succeeded; Charge `ch_3UA2jUGdq6QKDpuT1U0btylZ` captured, not refunded/disputed. One Session / one PI / one charge / `paymentCheckoutGeneration=1`. Webhooks `checkout.session.completed` `evt_1UA2jVGdq6QKDpuTXniWRtuq` and `payment_intent.succeeded` `evt_3UA2jUGdq6QKDpuT1ZJzkpQI` **processed** in `stripe_events`.
- Job TSK-3881 **FUNDED** / `paymentState=in_escrow` / `paymentStatus=succeeded`. Fee locked **standard_launch** 10% (Taskio **$9.00**, expert **$81.00**). Homeowner UI: no duplicate Accept/Continue CTA; payment secured. No release/transfer/payout/refund/dispute (`transferId`/`refundId` null).
- Auth user count **3**; `disabledUserSignup=true`; no OTP/SMS; no new users.
- **TSK-5507 / `HntNSWerak2NJreuvFlX`** was preserved funded/unreleased at B4F close; **released in B4G** (see B4G evidence below).
- Production Hosting still `cffca9d87ce03901`. gcloud default still `taskio-v2`.

**B4G evidence (2026-08-30) — PASS:**

- B4F evidence commit `66bb84cc4dcc2b45075fe922c6932a9940aed4c7` (`docs: record B4F hosted Checkout URL retest PASS`) **pushed** to `origin/develop` (no force). CI [`33301623028`](https://github.com/Taskio-au/Taskio-MVP/actions/runs/33301623028) **success** (standard jobs).
- Staging unchanged: API **100%** `taskio-api-staging-54aed8b` (previous `00063-qak` 0% retained); Hosting **`70429316be0dd106`**; Stripe TEST secret `:3`; signup **CLOSED**; Auth users **3**. Production Hosting still `cffca9d87ce03901`. gcloud default still `taskio-v2`.
- Read-only preflight: Expert **Mark task as completed** is `POST /api/jobs/:id/complete` (assigned tradie only; `FUNDED`/`IN_PROGRESS` + `in_escrow` → `COMPLETED` + `completedAt`; **no Stripe call**). Homeowner **Approve work & release payment** is `POST /api/jobs/:id/release` (homeowner only; requires `COMPLETED` + `in_escrow`). Transfer uses locked `feeSnapshot` (`expertNetCents`), `source_transaction` = captured charge, `transfer_group=taskio_job_{jobId}`, destination = Expert `stripeAccountId`, Stripe idempotency `taskio_release_{jobId}`. Persist is **synchronous** from the release endpoint (`status=PAID`, `paymentState=released`, `transferId`). App handles `transfer.reversed` as an incident; it does **not** persist release from `transfer.created`.
- Stripe Connect preflight: platform TEST `acct_1Rrhf7Gdq6QKDpuT`; Expert Express `acct_1U7VjdKCF5W6OUwD` (transfers **active**); no prior TSK-5507 transfer/refund/payout. Transfer is tied to captured charge `ch_3UA1mLGdq6QKDpuT1GYoEl3y` via `source_transaction` (does not require platform available balance).
- **B4G-A PASS:** existing synthetic Expert, hosted `/tradie/job/HntNSWerak2NJreuvFlX`, clicked **Mark task as completed** once. `POST /api/jobs/HntNSWerak2NJreuvFlX/complete` **200** “Task marked complete. Awaiting client approval.” Job → `COMPLETED` / `paymentState=in_escrow` / `completedAt=2026-08-30T08:34:01.475Z`. `transferId` still null. No Stripe Transfer / payout / refund. TSK-3881 untouched.
- Homeowner pre-release: hosted `/job/HntNSWerak2NJreuvFlX` showed **Ready for your approval**, **Payment secured**, **Approve work & release payment**, quote **$120.00**. API: gross **12000**, Taskio fee **1200**, expert net **10800**, no `transferId`/refund/dispute. (Homeowner job detail did not surface a `$108` line; locked snapshot was confirmed from GET job / Firestore.)
- **B4G-B PASS:** existing synthetic Homeowner clicked **Approve work & release payment** once. `POST /api/jobs/HntNSWerak2NJreuvFlX/release` **200** “Payment released successfully.” `totalProviderAmountCents=10800`. Release CTA removed (count **0**). Success copy: “You approved completion — payment is released. The Expert’s payout is processed by Stripe.” Review section appeared; **review was not submitted**.
- Stripe TEST Transfer (exactly one for TSK-5507):
  - ID: `tr_3UA1mLGdq6QKDpuT1g2pZQFm`
  - `livemode=false`; amount **10800** `aud`; destination **`acct_1U7VjdKCF5W6OUwD`**
  - `source_transaction=ch_3UA1mLGdq6QKDpuT1GYoEl3y`; `transfer_group=taskio_job_HntNSWerak2NJreuvFlX`
  - metadata: `type=job_release_base`, `jobId=HntNSWerak2NJreuvFlX`, `platformFeeAmount=1200`, `providerAmount=10800`
  - created `2026-08-30T08:37:27Z`; not reversed
- Taskio retained fee: **AUD 12.00** (locked `standard_launch` 10% snapshot unchanged).
- Event/idempotency: Stripe `transfer.created` `evt_3UA1mLGdq6QKDpuT1JInDFRc` (`livemode=false`, `pending_webhooks=0`) exists for this transfer. Taskio **did not** persist it in `stripe_events` (expected: release state is written by the release endpoint). Duplicate-release: UI CTA gone; if called again the API returns 200 “Payment already released.” with existing `transferId`; Stripe key `taskio_release_HntNSWerak2NJreuvFlX`. Stripe search: **one** matching transfer. Second CTA was **not** clicked.
- Final TSK-5507: `status=PAID`, `paymentState=released`, `paymentStatus=succeeded`, `transferId=tr_3UA1mLGdq6QKDpuT1g2pZQFm`, `releasedAt=2026-08-30T08:37:28.169Z`, `refundId` absent, quote `BCoGSMWsh7BGpdLdInlP` still **accepted** AUD 120, fee snapshot unchanged.
...
- **P03 OVERALL AT THIS 2026-09-04 CHECKPOINT:** STAGING PASS / PRODUCTION PENDING (superseded by P03G2 production PASS)
...
- **P03 EMAIL APPLICATION LOGIC: CODE COMPLETE.** **P03 LOCAL / CI TESTING: PASS** (`npm --prefix functions run lint`; `node --check functions/index.js`; `npm run test:functions` **26/26**; no real SMTP send). **P03 STAGING DELIVERY: NOT VERIFIED.** **P03 PRODUCTION: NOT CONFIGURED / NOT VERIFIED.**
...
**P05 evidence (2026-08-30) — GREEN App Check readiness; enforcement NOT ENABLED:**

- **P05 APP CHECK ARCHITECTURE: DECIDED.** **P05 APPLICATION CODE: CODE COMPLETE.** **P05 LOCAL / CI TESTING: PASS.** **P05 OVERALL: PARTIAL / READY FOR CONTROLLED STAGING ACTIVATION.**
- Selected future coverage: **Firestore** and **Storage**. Firebase Functions and Cloud Run API App Check are **not required for MVP**.
- Provider direction: reCAPTCHA **Enterprise** if clean/practical during AMBER staging setup; otherwise retain existing reCAPTCHA **v3**.
- Pushed application: `6b41bd3780bef5e6803bd1b4711665454ac9ec75`. Evidence: `695c2993638d77c0fb3e25926fa40ac833274eba`. CI for application push [`33308449769`](https://github.com/Taskio-au/Taskio-MVP/actions/runs/33308449769) **success**. CI for evidence push [`33308907690`](https://github.com/Taskio-au/Taskio-MVP/actions/runs/33308907690) **success**.

- Architecture: browser uses Auth, Firestore, and Storage. Functions are Firestore triggers only (no browser callables). Cloud Run stays on Firebase ID tokens + CORS + Helmet + rate limits; App Check middleware is **not** an MVP requirement (recommendation C).
- Frontend: existing A14 scaffold expanded with once-only init, optional `REACT_APP_APPCHECK_PROVIDER` (`recaptcha-v3` default, `recaptcha-enterprise` allowed), fail-closed missing site key, production/staging debug-token ban. Default remains `REACT_APP_APPCHECK_ENABLED` unset/false.
...
- P01–P06 evidence above is unchanged **as of this overlay**. P01 was **NOT PROVEN** here and later **PASS** on 2026-09-07. P06 remains the legal/privacy **review** gate (owner facts complete; lean sole-trader pilot as owner working plan subject to solicitor confirmation; Pty Ltd not an automatic blocker; not PASS). P09 implements P06 and cannot PASS before P06.
...
- Pushed: `e7ffbf0e6cfe992e61bbe71b01be2cda6145983a`. CI [`33310943590`](https://github.com/Taskio-au/Taskio-MVP/actions/runs/33310943590) **SUCCESS**.
- Provider: **GA4 via `gtag.js`**. Disabled by default. No PII; no job/user IDs in event payloads; no exact payment amounts (coarse amount buckets). Google Signals **off**; ad personalization **off**. No Meta Pixel, TikTok, Hotjar, FullStory, session replay, or Google Ads `AW-` tag.
...
**P04 URL privacy hardening (2026-09-06) — GREEN; now hosted on staging:**
...
- Caller-supplied `page_location` / `page_referrer` are ignored. Enhanced Measurement remains **OFF**.

**P04 staging Hosting activation (2026-09-06) — AMBER Hosting only:**

- Code origin `9d1119be08f8fa3178a7d4bf9ddcf051c4841b87`. CI [`34013435627`](https://github.com/Taskio-au/Taskio-MVP/actions/runs/34013435627) **success**.
- Taskio Staging GA4 property / stream configured by owner. Measurement ID `G-SZ7RZDKTJY` enabled on staging Hosting only. Owner privacy settings established (Enhanced Measurement OFF; Signals OFF; 2-month retention; ads personalization off; Google sharing off).
- Build + `scan:staging` **PASS**. Deployed bundle `main.9647f8fc.js`. Hosting version **`c2b8f742e73fed84`** (`2026-09-06T05:28:59.900Z`). Previous **`548438126950e209`**.
- Ride-along: `38c89d6` floating-shelves hero.
- **HOSTED NETWORK PROOF: PASS.** gtag.js only `G-SZ7RZDKTJY`. Config `send_page_view=false`, Signals off, ad personalization off, `anonymize_ip=true`. Observed `landing_viewed` once and `login_cta_clicked` once. Same-tab reload did not emit a second `landing_viewed`.
- Privacy: no email/phone/name/address/UID/job/quote/Stripe IDs/description/chat/DOB/ABN/exact amounts. `page_location` = `https://taskio-v2-staging.web.app/`. Same-origin job referrer → `/job/:id`. External referrer origin-only.
- **GA4 CONSOLE RECEIPT: PASS — OWNER CONFIRMED.** Taskio Staging Realtime: 1 active user; `landing_viewed` = 1; `login_cta_clicked` = 1; no automatic `page_view`. Normal GA4 `first_visit` and `session_start` were present and are not a failure.
- Production analytics **OFF**. Production Hosting still maintenance. No Cloud Run/Functions/Auth/App Check/Stripe/secret mutation. See `docs/ANALYTICS.md`.
...
**1. Core private MVP (GREEN landed; B4A–G PASS)**
...
| P03 | Essential transactional email (SMTP secrets) | **PRODUCTION PASS / COMPLETE / CLOSED.** Staging PASS. Production SMTP configured; sender/domain authorised; one guarded fixed-copy proof returned **200**, provider accepted, and owner receipt was confirmed. Proof gate restored off; final **404 `proof_disabled`**. Both E01 customer gates remain `EMAIL_ENABLED=false`. |
| P04 | GA4/provider setup for analytics (no ad pixels) | **PASS — OWNER-APPROVED PRODUCTION OFF.** Staging Hosting **`c2b8f742e73fed84`** loads `G-SZ7RZDKTJY`; hosted network and owner-confirmed Realtime receipt pass. Production remains OFF through P06/P09; no production proof is claimed. See `docs/ANALYTICS.md`. |
| P05 | Staging App Check validation + production enforcement decision | **PRODUCTION PASS / COMPLETE / CLOSED.** Firestore + Storage enforced/proven in production; Auth Monitoring / `UNENFORCED`; temporary proof surface removed. See `docs/APP_CHECK.md`. |
| P06 | Final Terms/Privacy owner + preferably AU legal review | **OPEN.** Owner facts **COMPLETE**. P06A reconciliation **PREPARED**. Lean sole-trader controlled pilot as owner working plan, subject to solicitor confirmation. Pty Ltd **not** an automatic launch blocker. Insurance = focused minimum-pilot review. Remediation **matrix prepared / implementation not started**. **Not PASS.** |
| P07 | Production security and configuration | **PASS / CLOSED.** Security/config readiness and owner activation boundaries are complete. Auth signup, GA4 and Stripe LIVE remain disabled while frozen; later activation/proof belongs to P10/P11. No final RED mutation. See `docs/P07_SECURITY_CONFIG_REMEDIATION.md` §48. |
...
| P10 | Production acceptance of money loop and critical failures | **NOT STARTED.** SPA/API/Authorization proof, controlled Auth enable/new-user/rollback proof, LIVE Stripe after P06/P09, customer email if required, and optional later-approved GA4 proof remain open. RED. See `docs/LAUNCH_READINESS.md`. |
...
> **Superseded for live staging topology 2026-08-30 (updated after B4F remediation).** Current serving API is `taskio-api-staging-54aed8b` (Checkout Session URL payload; closed signup; CORS for staging Hosting + localhost; TEST Stripe secret `:3`). Previous serving `00063-qak` is retained tagged. Historical closed-signup CORS revision `taskio-api-staging-00030-m9x` remains evidence only. Staging Hosting now serves `70429316be0dd106` (`54aed8b` SPA). This subsection remains the 2026-08-23 Stripe-lifecycle evidence snapshot.
...
| 2026-08-30 | `develop` / `54aed8b` | B4F | **PASS.** Pushed `c0a2c04`+`54aed8b`; CI `33297343725` success. API `taskio-api-staging-54aed8b` 100% (previous `00063-qak`). Hosting `70429316be0dd106` (previous `66888d3b5a527558`). Stripe.js removed; no pre-payment `js.stripe.com`; hosted Checkout uses server `checkoutUrl`. New job `fu8pWO3D4BY4DCo1s36u` / TSK-3881; quote `CqywhWbZZWARfEDBoW6D` AUD 90 accepted; automatic Checkout redirect PASS; Session `cs_test_a1Z1lRIPA155ZS1XiIhT5W4Q2xxLPIZq8Dk6vQgPoUNkTfxQBg7Q1ZD3xC` paid `livemode=false`; PI `pi_3UA2jUGdq6QKDpuT1mXg2gyb`; Charge `ch_3UA2jUGdq6QKDpuT1U0btylZ`; webhooks processed; job `FUNDED` / `in_escrow`; fee standard_launch 10% ($9/$81). No duplicate payment. Auth count 3. Signup closed. **No release/transfer/payout/refund.** TSK-5507 preserved funded/unreleased. Production Hosting `cffca9d87ce03901`. | Hosted Playwright; Cloud Build `801126b4-83c1-4937-aca2-565872d12852`; Hosting wrapper; bundle scan; Stripe retrieve; Firestore/`stripe_events`; Identity Toolkit; Cloud Run describe | API+Hosting staging deploy. Tracker evidence local, **not pushed** until another approval | Next: separately approved release/Connect transfer on TSK-5507 |
