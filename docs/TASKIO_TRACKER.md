...
**2026-09-25 P05G1 production App Check and SPA readiness plan (GREEN local docs): COMPLETE.** Provider is Firebase JS App Check; staging used reCAPTCHA Enterprise. Required production services are Firestore and Storage. Auth App Check is deferred. Auto-refresh is on. Production debug tokens are forbidden. Production site-key registration is **MISSING**. **CODE / CLIENT-INITIALIZATION BLOCKER:** the first App Check token is not acquired before the first Firestore or Storage call. **SEPARATE PRODUCTION BROWSER/API ARCHITECTURE BLOCKER:** the SPA needs `REACT_APP_API_BASE_URL`, the production API is IAM-private, and App Check does not solve that. Full SPA restoration is not approved for P05. Next GREEN work is **P05G1A** then **P05G1B**. RED enforcement is not approved until both pass. GA4 stays off. Auth signup stays disabled. Hosting stays maintenance. P05 remains **STAGING PASS / PRODUCTION PENDING**.
