...
**2026-09-13 Pilot Expert readiness criteria tightened (GREEN docs only):** Launch-ready for ~15 / ~12 = technical eligibility **AND** `acceptingJobs=true` **AND** `serviceAreas[]` contains ≥1 enabled canonical pilot area. Those two fields are **required before real homeowner posting opens** (small profile/data changes; **not implemented**). No stored `launchReady` boolean. Geography = **one Inner Melbourne zone**; hard **4–5** applies to **enabled Phase 1 categories**, not every suburb; do not treat home-base `serviceLocation` as service coverage. Cockpit counts must use this derived definition. No calendars/GIS/maps/radius. No product code. Production untouched.
