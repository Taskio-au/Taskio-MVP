...
**Status:** **P07 OPEN / REMEDIATION IN PROGRESS** — **AMBER-E2A + E2B + E2C + D3 COMPLETE**. **P07F1–F5B COMPLETE.** **P07G1–G3 COMPLETE.** **P07H1–H2 COMPLETE.** **P05G2 COMPLETE**; **P05G3 FAILED SAFELY / ROLLED BACK**; **P05G3B** proof CSP fix local only (enforcement OFF). Staging CSP **ENFORCED.** Expert expertise fail-open **FIXED + STAGING PROVEN + PRODUCTION LIVE** on `taskio-api-00008-zir` (source `57505d0`). Compatibility **REVIEW COMPLETE** (OPTION B). Production Hosting still maintenance. Production Firestore + Storage rules now match HEAD (§47). **Not P07 PASS.** READY TO OPEN remains impossible. Production still **FROZEN** (not open-demand).
...
| Production | **Provider registered; P05G3 rolled back to maintenance; P05G3B CSP fix local only** | UNENFORCED | UNENFORCED | UNENFORCED |

**API:** no App Check middleware (intentional MVP).

**Current production state (P05G3 / P05G3B):** the existing production Web app remains registered with one domain-restricted SCORE-based reCAPTCHA Enterprise key. **P05G3 FAILED SAFELY / ROLLED BACK:** the proof-page CSP blocked `https://apis.google.com/js/api.js` and Firebase Auth returned `auth/internal-error`. Live Hosting is maintenance-only (`cffca9d87ce03901`). **P05G3B** adds that host to the local proof-page `script-src` only and is not deployed. Firestore, Storage, and Auth remain UNENFORCED. Not P05G3 PASS.
...
| P07-06 | Production App Check | **P05G3 FAILED SAFELY / ROLLED BACK.** Provider remains registered. Proof CSP blocked `https://apis.google.com/js/api.js` (`auth/internal-error`). **P05G3B** script-src fix is local only. Firestore/Storage/Auth `UNENFORCED` | C5 | HIGH bots/abuse at public launch | Fresh P05G3 retry, then P05G4–P05G6, each under separate RED approval | PRODUCTION | **RED IN PROGRESS** | **Yes** (with P05) | Hosted token + Firestore/Storage enforcement proofs | **NOT P05G3 PASS; ENFORCEMENT NOT STARTED** |
...
| P07-06 | Production App Check | **P05G3 FAILED SAFELY / ROLLED BACK.** Provider registered. **P05G3B** script-src fix local only. Firestore/Storage/Auth UNENFORCED | **Yes** (P05) for public launch | deploy/prove/enforce | Fresh P05G3 retry after the local CSP fix | RED P05 sequence (Hosting → Firestore → Storage) | **RED IN PROGRESS** | with Hosting |
