...
const { categoryRequiresPostingPhoto, includesMirrorWork } = require('../shared/jobPostingSemantics');
...
function postingRequiresPhoto(primaryCategory, includesMirror, details) {
  return categoryRequiresPostingPhoto(primaryCategory)
...
router.post('/api/jobs', requireAuth, requireEnrolledProfile({
  ...
  try {
    ...
    const includesMirror = includesMirrorWork(normalizedItems.items, normalizedItems.primaryCategory);
    ...
    const postingPhotoRequired = postingRequiresPhoto(
      normalizedItems.primaryCategory,
      includesMirror,
      normalizedDetails
    );
...
router.post('/api/jobs/:id/photos', requireAuth, requireEnrolledProfile({
  ...
  try {
    ...
    if (jobData.postingPhotoRequired === true && photos.length === 0) {
...
router.post(
  ...
  try {
    ...
    if (jobData.postingReady === false) {
      return res.status(409).send({ message: 'This task is not ready for quotes until its required photos are uploaded.' });
    }
    ...
    if (categoryMatch.reliable && !categoryMatch.matches) {
      return res.status(403).send({
        message: 'Task expert is not approved for this task category.',
        code: 'TRADIE_NOT_ELIGIBLE',
        reasons: ['EXPERTISE_NOT_APPROVED_FOR_JOB'],
      });
    }
